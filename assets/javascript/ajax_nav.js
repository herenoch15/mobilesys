    ...
    function ajaxLoad () {
        ...
        switch (nStatus) {
            case 200:
			   ...
			   //$("#body_pageCenter").css("display","block");
				//alert(this.responseText);
				//document.getElementById(sTargetId).innerHTML = "vMsg.content";
				
				/*$("#body_pageCenter").load(function ()
					{
						$(this)(vMsg.content);
					}
				);*/
				//.load(oPageInfo.url+"?view_as=json");
				//alert(oPageInfo.url);
				//$("#body_pageCenter").load(vMsg.content);
                if (bUpdateURL) {
					
                   //alert(oPageInfo.url);
				   change_lienMenue(oPageInfo.url);
  ...
  //alert("trouvé!");
    ...
    function init () 
	{
        	...
        	oPageInfo.title = document.title;
           	$(document).on("click", "a, area",processLink);
	}

    var

        /* customizable constants */
        sTargetId = "body_pageCenter", sViewKey = "view_as", sAjaxClass = "ajax-nav",
